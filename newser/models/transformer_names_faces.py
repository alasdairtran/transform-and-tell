...
@Model.register("transformer_names_faces")
class TransformerNamesFacesModel(Model):
    """
    ...
    def __init__(self,
        ...
        self._endpoint_span_extractor = EndpointSpanExtractorFixed(
            1024,
            combination="x,y",
            num_width_embeddings=20,
            span_width_embedding_dim=20,
            bucket_widths=False)

        self._attentive_span_extractor = SelfAttentiveSpanExtractorFixed(
            input_dim=1024)

        self.names_proj = GehringLinear(1024*3 + 20, 1024)
    ...
    def _forward(self,  # type: ignore
        ...
        # Get name embeddings. Note that span ends are inclusive
        name_spans = context['name_spans']
        # name_spans.shape == [batch_size, n_spans, 2]

        name_masks = (name_spans == -1).any(dim=-1)
        # name_masks.shape == [batch_size, n_spans]

        # We'll generate dummy representations for the padding spans. We need
        # to remember to exclude them later on with name_masks
        name_spans[name_spans == -1] = 0

        # Each span is the concatenation of the embedding of two endpoints plus
        # the embedding of span width.
        endpoint_span_embeddings = self._endpoint_span_extractor(
            X_article, name_spans)
        # endpoint_span_embeddings.shape == [batch_size, n_spans, embed_size * 2 + feature_size]

        # We do attention on the spans. For each span, we compute a weighted
        # sum of the word embeddings in the span. The weight is the attention
        # score and is simply the softmax of a linear projection of X.
        attended_span_embeddings = self._attentive_span_extractor(
            X_article, name_spans)
        # attended_span_embeddings.shape == [batch_size, n_spans, embed_size]

        name_embeddings = torch.cat([endpoint_span_embeddings,
                                     attended_span_embeddings], -1)
        # name_embeddings.shape == [batch_size, n_spans, embed_size * 3 + feature_size]

        name_embeddings = self.names_proj(name_embeddings)
        ...
        contexts = {
            ...
            'sections': None,
            'sections_mask': None,
            'names': name_embeddings.transpose(0, 1),
            'names_mask': name_masks,
