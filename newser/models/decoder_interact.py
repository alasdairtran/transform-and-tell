...
@Decoder.register('dynamic_conv_decoder_interact')
class DynamicConvInteractDecoder(Decoder):
    """
...
@DecoderLayer.register('dynamic_conv_interact')
class DynamicConvDecoderLayer(DecoderLayer):
    ...
    def __init__(self, decoder_embed_dim, decoder_conv_dim, decoder_glu,
        ...
        self.context_attns['article'] = MultiHeadAttention(
            self.embed_dim * 2, decoder_attention_heads, kdim=1024, vdim=1024,
            dropout=attention_dropout)
        self.context_attn_lns['article'] = nn.LayerNorm(self.embed_dim * 2)
    ...
    def forward(self, X, contexts, incremental_state):
        ...
        X_with_image = torch.cat([X, X_image], dim=-1)
        residual = X_with_image
        X_article = self.maybe_layer_norm(
            self.context_attn_lns['article'], X_with_image, before=True)
