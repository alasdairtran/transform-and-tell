...
@Decoder.register('dynamic_conv_decoder_faces_parallel_2')
class DynamicConvFacesParallel2Decoder(Decoder):
    """
...
@DecoderLayer.register('dynamic_conv_faces_parallel_2')
class DynamicConvDecoderLayer(DecoderLayer):
    ...
    def __init__(self, decoder_embed_dim, decoder_conv_dim, decoder_glu,
        ...
        self.fc1 = GehringLinear(self.embed_dim * 3, decoder_ffn_embed_dim)
    ...
    def forward(self, X, contexts, incremental_state):
        ...
        X = self.maybe_layer_norm(
            self.final_layer_norm, X_context, before=True)
        X = F.relu(self.fc1(X))
