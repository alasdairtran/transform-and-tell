...
@Model.register("transformer_summary")
class TransformerSummaryModel(Model):
    """
    ...
    def _forward(self,  # type: ignore
        ...
        summary_ids = full_context[self.index]
        whole_article_padding_mask = summary_ids == self.padding_idx
        # whole_article_padding_mask.shape == [batch_size, max_sections, seq_len]

        B, G, S = summary_ids.shape
        summary_ids = summary_ids.reshape(B * G, S)
        # summary_ids.shape == [batch_size * max_sections, seq_len]

        # Remove empty sections (those containing only padding)
        section_mask = ~(summary_ids == self.padding_idx).all(dim=1)

        X_sections_hiddens = self.roberta.extract_features(
            summary_ids[section_mask], return_all_hiddens=True)

        if self.weigh_bert:
            X_last = [x[:, 0] for x in X_sections_hiddens]
            X_summary = torch.stack(X_last, dim=1)
            # X_article.shape == [batch_size_full, 25, embed_size]

            weight = F.softmax(self.bert_weight_2, dim=0)
            weight = weight.unsqueeze(0).unsqueeze(2)
            # weight.shape == [1, 25, 1]

            X_summary = (X_summary * weight).sum(dim=1)
            # X_article.shape == [batch_size_full, embed_size]

        else:
            X_summary = X_sections_hiddens[-1][:, 0]
            # X_sections.shape == [batch_size * n_sections, embed_size]

        X_sections = X_image.new_full((B * G, 1024), self.padding_idx)
        X_sections[section_mask] = X_summary
        X_sections = X_sections.view(B, G, -1)
        # X_sections.shape == [batch_size, max_sections, embed_size]

        # Create padding mask (1 corresponds to the padding index)
        image_padding_mask = X_image.new_zeros(B, P).bool()
        section_padding_mask = whole_article_padding_mask.all(dim=-1)
        ...
        contexts = {
            ...
            'summary': X_sections.transpose(0, 1),
            'summary_mask': section_padding_mask,
        }
