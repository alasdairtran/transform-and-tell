...
@DatasetReader.register('goodnews_coref')
class CorefGoodNewsReader(DatasetReader):
    ...
    def article_to_instance(self, article, image, image_index, clusters) -> Instance:
        ...
        for cluster in clusters:
            if cluster['pos'] != 'PROPN' or not cluster['article_corefs']:
                continue

            relevant_corefs = [c for c in cluster['article_corefs']
                               if c['pos'] == 'PROPN']
            ...
            for c in cluster['caption_corefs']:
                if c['pos'] == 'PROPN':
                    caption_copy_pos.append((c['start'], c['end']))

            for c in relevant_corefs:
                context_copy_pos.append((c['start'], c['end']))

        fields = {
            'context': CorefTextField(context_tokens, self._token_indexers, context_copy_pos),
            'image': ImageField(image, self.preprocess),
            'caption': CorefTextField(caption_tokens, self._token_indexers, caption_copy_pos),
        }
