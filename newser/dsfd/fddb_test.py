...
def test_fddbface():
    ...
    all_splits = sorted([_ for _ in os.listdir(
        args.split_dir) if 'ellipseList' not in _])
    for folder_ind in range(1, 11):
        ...
        with open(os.path.join(args.det_dir, 'fold-{:02d}-out.txt'.format(folder_ind)), 'wt') as f:
            ...
            for image_ind, image_name in enumerate(all_images):
                sys.stdout.write('\r>> Predicting image %d/%d' %
                                 (image_ind, all_image_length))
                sys.stdout.flush()
                #np_image = imread(os.path.join(data_dir, image_name+'.jpg'))

                np_image = cv2.imread(os.path.join(
                    args.data_dir, image_name+'.jpg'))
                ...
                img_to_draw = draw_toolbox.absolute_bboxes_draw_on_img(
                    np_image, scores, dets, thickness=2)
                imsave(os.path.join('./debug/{}.jpg').format(image_ind), img_to_draw)

                valid_mask = np.logical_and(np.logical_and(
                    (bbox_height > 1), (bbox_width > 1)), (scores > 0.05))
                ...
                for det_ind in range(valid_mask.shape[0]):
                    ...
                    f.write('{:.1f} {:.1f} {:.1f} {:.1f} {:.3f}\n'.format(np.floor(bbox_xmin[det_ind]), np.floor(
                        bbox_ymin[det_ind]), np.ceil(bbox_width[det_ind]), np.ceil(bbox_height[det_ind]), scores[det_ind]))
        sys.stdout.write('\n')
        sys.stdout.flush()
