...
from tell.dsfd.data import widerface_640 as cfg
...
class Detect(Function):
    ...
    """
    @staticmethod
    def forward(ctx, loc_data, conf_data, prior_data, num_classes, background_label,
                top_k, conf_thresh, nms_thresh, arm_loc_data=None,
                arm_conf_data=None):
        """
        Args:
        ...
        for i in range(num):
            if cfg['refinedet']:
                #default  = center_size(decode(arm_loc_data[i] , prior_data , variance))
                decoded_boxes_arm = decode(
                    arm_loc_data[i], prior_data, variance)
                default = center_size(decoded_boxes_arm)
                decoded_boxes_odm = decode(loc_data[i], default, variance)
