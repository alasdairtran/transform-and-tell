...
\begin{abstract}
   ...
   By combining the transformer architecture, byte-pair encoding, RoBERTa
   embeddings for text, ResNet-152 embeddings for images, and bags of
   attentions, our system is able to describe an image with specific named
   entities mentioned in the article. Our model achieves a CIDEr score of 54 on
   the GoodNews dataset, significantly outperforming the previous state-of-art
   CIDEr of 13. We also introduce the NYTimes+ dataset, an extended version of
   GoodNews with higher-quality articles and metadata that allow us to study
   the importance of the image location within the text. On NYTimes+, we
   achieve a CIDEr of 51. Pretrained models and source code are available from
...
image \cite{Vinyals2014ShowAT, Fang2014FromCT, Karpathy2015DeepVA,
Rennie2016SelfCriticalST, Lu2016KnowingWT, Anderson2017BottomUpAT}, mainly
...
newspapers. To the best of our knowledge, all of the existing models
that are trained on these news datasets use template-based methods to insert
named entities.
...
contextual cues from different modalities (image and text) and 2) generate
captions that contain specific information including rare proper names that
have not been seen in the training data. To answer this, we look at the recent
development in sequence-to-sequence architectures and language modeling.
...
\begin{enumerate}
   ...
   \item We build on the GoodNews dataset to produce a larger and more complete
   news captioning dataset consisting of 445,828 articles from the New York
   Times and their 794K images with captions. This constitutes the largest
   news image captioning dataset to date. In contrast to GoodNews our dataset
