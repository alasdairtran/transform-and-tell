...
\cite{Biten2019GoodNews} include real-life captions written by professional
journalists. However, all existing models that are trained on these news
datasets use template-based methods to insert named entities.
...
the same period as GoodNews from the original source. We call this GoodNews+.
We were able to crawl all the original articles, but the number of images is
slightly less, either due to dead links or not crawling images unrelated to the
main article. GoodNews+ allows us to make the following observations about the
original GoodNews dataset:
