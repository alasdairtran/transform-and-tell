...
Despite the above limitations, we still use the original GoodNews dataset in
our experiments, along with the original train-validation-test split, to make
...
\begin{table}[t]
	...
	\begin{tabular}{lll}
		...
		\midrule
      Number of articles & 257 033 & 445 828 \\
      Number of images   & 462 642 & 794 085 \\
      ...
      \quad -- person names & 9\% & 9\% \\
      \midrule
      \% of captions with \\
      \quad -- named entities & 97\% & 97\% \\
      \quad -- person names & 68\% & 68\% \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{itemize}
   ...
   article.
   \item Unlike GoodNews, we also collect information about the image position
   in each article. Most news articles have one image at the top that relates
   to the key topic. However 39\% of the articles have at least one more image
   somewhere in the middle of text. The image placement and hence the text
   surrounding the image could be important information for captioning.
\end{itemize}

Named entities play an important role in the dataset, with 97\% of captions
containing at least one named entity. As shown in Figure \ref{fig:entity_dist},
the most popular entity type are person names, with 68\% of the captions
mentioning at least one person.
...
\begin{figure}[t]
   ...
   \end{center}
      \caption{Distribution of entity types in NYTimes800k training captions.}
...
\begin{table*}[t]
	...
	\begin{tabular}{llllllll}
      ...
      Transformer (first 512 tokens) & 20.7 & 12.4 & 8.14 & 5.73 & 19.8 & 9.54 & 44.1 \\ % Final
      Transformer (location-aware) & 21.7 & 13.3 & 8.84 & 6.25 & 21.3 & 10.3 & 52.4 \\ % Final
      \\
		\bottomrule
	\end{tabular}
\end{table*}
