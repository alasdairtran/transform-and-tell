...
dataset on news image captioning~\cite{Biten2019GoodNews}. Each example in the
dataset is a triplet containing an article, an image, and a caption. Since only
the article text, captions, and image URLs were publicly released
the images had to be downloaded from the original source. Out of the 466K image
URLs provided by
...
train-validation-test split provided by~\cite{Biten2019GoodNews}. This split
consists of 421K training, 18K validation, and 23K test captions. Note that
this split was done on captions, so it is possible for a training and test
...
We also constructed the NYTimes800k which is a larger and more complete dataset
...
GoodNews dataset, collect a larger dataset, and include fine grained context
such as the images location in the article.

We observed that many of the articles that constitute the GoodNews dataset had
only been partially extracted since the generic article extractor employed
failed to recognise certain HTML tags used
specifically by the New York Times. To collect the full articles we
...
After this recollection, we observe that the average article is 963 words while
the the average length of an article in
GoodNews is 451 words. Importantly the missing text may include the first few
paragraphs
which often contain important information for captioning images.

In addition, we also found that GoodNews contained a
...
A comparison between GoodNews and NYTimes800k is
