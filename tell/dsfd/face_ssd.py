...
class FEM(nn.Module):
    def __init__(self, channel_size):
        ...
        self.cpm1 = nn.Conv2d(self.cs, 256, kernel_size=3,
                              dilation=1, stride=1, padding=1)
        self.cpm2 = nn.Conv2d(self.cs, 256, kernel_size=3,
                              dilation=2, stride=1, padding=2)
        self.cpm3 = nn.Conv2d(256, 128, kernel_size=3,
                              dilation=1, stride=1, padding=1)
        self.cpm4 = nn.Conv2d(256, 128, kernel_size=3,
                              dilation=2, stride=1, padding=2)
        self.cpm5 = nn.Conv2d(128, 128, kernel_size=3,
                              dilation=1, stride=1, padding=1)
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, num_classes):
        ...
        if backbone == 'vgg':
            ...
            self.layer1 = nn.Sequential(
                detnet.conv1, detnet.bn1, detnet.relu, detnet.maxpool, detnet.layer1)
            ...
            self.layer5 = nn.Sequential(
                detnet.layer5)  # add one layer, for c6s
            ...
            if backbone == 'resnet101':
                ...
                resnet = pretrainedmodels.__dict__[
                    'se_resnext101_32x4d'](pretrained='imagenet')
        ...
        if fpn:
            ...
            self.latlayer3 = nn.Conv2d(
                fpn_in[3], fpn_in[2], kernel_size=1, stride=1, padding=0)
            self.latlayer2 = nn.Conv2d(
                fpn_in[2], fpn_in[1], kernel_size=1, stride=1, padding=0)
            self.latlayer1 = nn.Conv2d(
                fpn_in[1], fpn_in[0], kernel_size=1, stride=1, padding=0)
            ...
            self.smooth3 = nn.Conv2d(
                fpn_in[2], fpn_in[2], kernel_size=1, stride=1, padding=0)
            self.smooth2 = nn.Conv2d(
                fpn_in[1], fpn_in[1], kernel_size=1, stride=1, padding=0)
            self.smooth1 = nn.Conv2d(
                fpn_in[0], fpn_in[0], kernel_size=1, stride=1, padding=0)
        ...
        if bup:
            ...
            self.bup3 = nn.Conv2d(
                bup_in[2], bup_in[3], kernel_size=3, stride=2, padding=1)
            self.bup4 = nn.Conv2d(
                bup_in[3], bup_in[4], kernel_size=3, stride=2, padding=1)
            self.bup5 = nn.Conv2d(
                bup_in[4], bup_in[5], kernel_size=3, stride=2, padding=1)
        if fem:
    ...
    def forward(self, x):
        ...
        if refine:
            arm_loc = list()
            arm_conf = list()
            arm_sources = [conv3_3_x, conv4_3_x,
                           conv5_3_x, fc7_x, conv6_2_x, conv7_2_x]
        ...
        if pa:
            mbox_num = cfg['mbox'][0]
            face_loc = torch.cat(
                [o[:, :, :, :4*mbox_num].contiguous().view(o.size(0), -1) for o in loc], 1)
            face_conf = torch.cat(
                [o[:, :, :, :2*mbox_num].contiguous().view(o.size(0), -1) for o in conf], 1)
            head_loc = torch.cat(
            ...
            body_loc = torch.cat(
                [o[:, :, :, 8*mbox_num:].contiguous().view(o.size(0), -1) for o in loc[2:]], 1)
            body_conf = torch.cat(
                [o[:, :, :, 4*mbox_num:].contiguous().view(o.size(0), -1) for o in conf[2:]], 1)
        else:
    ...
    def mio_module(self, each_mmbox, len_conf):
        ...
        bmax = torch.max(torch.max(chunk[0], chunk[1]), chunk[2])
        cls = (torch.cat([bmax, chunk[3]], dim=1) if len_conf ==
               0 else torch.cat([chunk[3], bmax], dim=1))
        if len(chunk) == 6:
            cls = torch.cat([cls, chunk[4], chunk[5]], dim=1)
        elif len(chunk) == 8:
            cls = torch.cat(
                [cls, chunk[4], chunk[5], chunk[6], chunk[7]], dim=1)
        return cls
...
def add_extras(extras_cfg, i, batch_norm=False):
    ...
    for k, v in enumerate(extras_cfg):
        if in_channels != 'S':
            if v == 'S':
                layers += [nn.Conv2d(in_channels, extras_cfg[k + 1],
...
def multibox(output_channels, mbox_cfg, num_classes):
    ...
    for k, v in enumerate(output_channels):
        ...
        loc_layers += [nn.Conv2d(input_channels,
                                 mbox_cfg[k] * 4, kernel_size=3, padding=1)]
        if mo:
            if k == 0:
                conf_layers += [nn.Conv2d(input_channels,
                                          mbox_cfg[k] * 4, kernel_size=3, padding=1)]
            else:
                conf_layers += [nn.Conv2d(input_channels, mbox_cfg[k]
...
def pa_multibox(output_channels, mbox_cfg, num_classes):
    ...
    for k, v in enumerate(output_channels):
        ...
        if k == 0:
            ...
            loc_output = 12
            conf_output = 6
        loc_layers += [DeepHeadModule(input_channels,
                                      mbox_cfg[k] * loc_output)]
        if mio:
            conf_layers += [DeepHeadModule(input_channels,
                                           mbox_cfg[k] * (2+conf_output))]
        else:
            conf_layers += [DeepHeadModule(input_channels,
                                           mbox_cfg[k] * conf_output)]
    return (loc_layers, conf_layers)
