...
from tell.dsfd.data.config import widerface_640
...
class RandomBaiduCrop(object):
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        image = cv2.resize(image, None, None, fx=ratio,
        ...
        if len(sample_boxes) > 0:
            ...
            if choice_box[0] < 0 or choice_box[1] < 0:
                new_img_width = width if choice_box[0] >= 0 else width - \
                    choice_box[0]
                ...
                current_image = image_pad[start_top:end_bottom,
                                          start_left:end_right, :].copy()
                return current_image, current_boxes, current_labels
            current_image = image[choice_box[1]:choice_box[3],
                                  choice_box[0]:choice_box[2], :].copy()
            return current_image, current_boxes, current_labels
        else:
            return image, boxes, labels
            ...
            for _ in range(50):
                if mode is None or mode < 0.7:
                    if mode is None:
                        ...
                        w = random_random.randrange(
                            int(0.3*short_size), int(1*short_size))
                    h = w
                    if imw > w:
                    ...
                    if w > 256:
                        random_scale = 640 / \
                            random_random.choice([16, 32, 64, 128, 256, 512])
                    elif w > 128:
                        random_scale = 640 / \
                            random_random.choice([16, 32, 64, 128, 256])
                    elif w > 64:
                        random_scale = 640 / \
                            random_random.choice([16, 32, 64, 128])
                    elif w > 32:
                ...
                boxes_uniform = selected_boxes / \
                    torch.FloatTensor([w, h, w, h]).expand_as(selected_boxes)
                boxwh = boxes_uniform[:, 2:] - boxes_uniform[:, :2]
                #mask = (boxwh[:,0] > self.small_threshold) & (boxwh[:,1] > self.small_threshold)
                mask = (boxwh[:, 0]*w*boxwh[:, 1]*h >
